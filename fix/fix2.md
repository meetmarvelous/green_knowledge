Here's the complete updated code for both `admin/trees/add.php` and `admin/trees/edit.php`:

### 1. `admin/trees/add.php` (Complete Updated Version)
...
### 2. `admin/trees/edit.php` (Complete Updated Version)
...
1. **Data Structure**: Updated to match the new table structure with `qr_code_path` instead of `pictures`
2. **Form Fields**: Added all new fields (`common_name`, `description`, `ecological_info`, etc.)
3. **Photo Handling**: Separated tree photos (multiple) from QR code (single)
4. **Validation**: Proper validation for all fields
5. **Error Handling**: Comprehensive error logging and user feedback
6. **Session Management**: Proper session handling with `session_write_close()`

Both files now perfectly match your updated database structure and provide complete CRUD functionality for all tree data fields.